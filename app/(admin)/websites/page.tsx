...
export default function WebsitesPage() {
  ...
  if (!openCompany) {
    ...
    return (
      <div>
        <PageHeader title={t('page.websites.title')} description={t('page.websites.description')} actions={<ViewToggle value={viewMode} onChange={setViewMode} />} />
          ...
          <div className="rounded-xl border bg-white overflow-hidden" style={{ borderColor: '#e2e8f0' }}>
            {filteredCompanies.map((company, i) => (
              <Link
                ...
                style={{ borderBottom: i < filteredCompanies.length - 1 || filteredUnassigned.length > 0 ? '1px solid #f1f5f9' : 'none' }}
              >
                ...
                <div className="flex items-center gap-3 flex-shrink-0">
                  <span className="text-xs" style={{ color: '#64748b' }}>{company.site_count} {company.site_count === 1 ? 'site' : 'sites'}</span>
                  {!isWriter && company.total_phones > 0 && <span className="text-xs" style={{ color: '#94a3b8' }}>{company.total_phones} phones</span>}
                  {company.total_blogs > 0 && <span className="text-xs" style={{ color: '#94a3b8' }}>{company.total_blogs} posts</span>}
            ...
            {filteredUnassigned.length > 0 && (
              <div className="flex items-center gap-3 px-4 py-3.5">
                ...
                <span className="text-xs flex-shrink-0" style={{ color: '#94a3b8' }}>{filteredUnassigned.length} {filteredUnassigned.length === 1 ? 'site' : 'sites'}</span>
              </div>
            )}
          </div>
        )}
